...
import 'package:lil_footprint/local/place_cache_db.dart';
import 'package:lil_footprint/platform_exceptions.dart';
...
// Protomaps style URL (can be overridden via constructor)
const String defaultStyleUrl =
    'https://api.protomaps.com/styles/v5/light/zh-Hant.json?key=2ed98bca16a38d12';
...
class PlacesMapView extends StatefulWidget {
  ...
  /// The map style that will be given to MapLibre.  If null the built‑in
  /// Protomaps style is used.
...
class _PlacesMapViewState extends State<PlacesMapView> {
  ...
  // Keep track of the circle that corresponds to the currently selected
  // PlaceItem so that we can highlight it later.
  ...
  // 是否使用「手動更新地圖」模式（關閉即時更新以降低負載）
  bool _manualUpdateMode = false;
  ...
  // 額外條件：設備 / 資料新舊
  bool _onlyDiaperTable = false;
  bool _onlyParking = false;
  bool _includeOutdated = false;
  ...
  void initState() {
    super.initState();
    _getCurrentLocation();
    // camera changes are handled via the MapLibreMap callbacks (see build()).
  }
  ...
  Future<void> _openDirections(PlaceItem item) async {
    ...
    if (lat.isNaN || lng.isNaN) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('座標無效，無法開啟地圖')));
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('無法開啟地圖：$e')));
    }
  }
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      if (_mapController != null) {
        _mapController!
            .animateCamera(CameraUpdate.newLatLngZoom(userLocation, 15.0));
      }
  ...
  // 計算當前地圖視窗的 bbox
  Future<String> _currentBboxString() async {
    if (_mapController == null) return '';
    final bounds = await _mapController!.getVisibleRegion();
    // MapLibre returns southwest / northeast
    return '${bounds.southwest.longitude},${bounds.southwest.latitude},'
  ...
  void _zoomIn() {
    ...
    _mapController!
        ...
        .then((_) {
      if (!_manualUpdateMode) _fetchForCurrentView();
    });
  }
  ...
  /// Update the circles shown on the map to reflect [_items].
  ///
  /// Uses pin-like dots (circles) for place markers.
  Future<void> _updateAnnotations() async {
    if (_mapController == null) return;

    await _mapController!.clearCircles();

    // add a dot (circle) for each item
    final options = _items.map((item) {
      final colorHex =
          '#${_categoryColor(item.category).value.toRadixString(16).padLeft(8, '0').substring(2)}';
      return CircleOptions(
        geometry: LatLng(item.lat, item.lng),
        circleRadius: 8,
    ...
    final data = _items.map((item) => {'id': item.id}).toList();

    final circles = await _mapController!.addCircles(options, data);

    // if something was selected before the update, highlight its circle
    if (_selected != null && circles.isNotEmpty) {
      try {
        _selectedCircle =
            circles.firstWhere((c) => c.data?['id'] == _selected!.id);
  ...
  Future<List<PlaceItem>> _fetchPlacesMulti({
    ...
    String? city,
    String? district,
    bool onlyDiaperTable = false,
    bool onlyParking = false,
    ...
    final queryParameters = <String, List<String>>{
      ...
      if (onlyDiaperTable) "has_diaper_table": ['1'],
      if (onlyParking) "has_parking": ['1'],
      if (includeOutdated) "include_outdated": ['true'],
    };
  ...
  bool _hasKindergartenOrDaycareCategory() {
    ...
    return selected.any(
      (cat) => cat.contains('kindergarten') || cat.contains('daycare'),
    );
  }
  ...
  Future<void> _fetchAvailableCities({bool forceRefresh = false}) async {
    ...
    try {
      // 構建查詢參數
      final queryParameters = <String, List<String>>{"category": cats};
      final query = queryParameters.entries
          .expand(
            (entry) => entry.value.map(
              (value) =>
  ...
  Future<void> _fetchForCurrentView() async {
    ...
    final String? bbox = useBbox ? await _currentBboxString() : null;
    ...
    try {
      ...
      final items = await _fetchPlacesMulti(
        ...
        onlyDiaperTable: _onlyDiaperTable,
        onlyParking: _onlyParking,
        includeOutdated: _includeOutdated,
      );
                              ...
                              children: [
                                Wrap(
                                  ...
                                  children:
                                    ...
                                    // make all category chips same width
                                    return SizedBox(
                                      width: 140,
                                      child: FilterChip(
                                        label: Center(
                                ...
                                SwitchListTile(
                                  ...
                                  title: const Text('手動更新地圖（關閉即時更新）'),
                                  subtitle: const Text('關閉即時載入可降低 API 與地圖負載'),
                                  value: _manualUpdateMode,
                                  onChanged: (value) {
                                    setState(() {
                                      _manualUpdateMode = value;
                                    });
                                    setModalState(() {});
                                    // 從手動模式切回即時模式時，立即更新一次地圖
                                    if (!value) {
                                      _fetchForCurrentView();
                                    }
                                  },
      ...
      children: [
        ...
        Positioned.fill(
          child: MapLibreMap(
            ...
            styleString: widget.styleUrl ?? defaultStyleUrl,
            onMapCreated: (ctrl) {
              _mapController = ctrl;
              // Register circle (dot) tap listener
              ctrl.onCircleTapped.add((circle) {
                final data = circle.data;
                if (data != null && data['id'] != null) {
                  try {
                    final item = _items.firstWhere((p) => p.id == data['id']);
                    setState(() {
                      _selected = item;
                      _selectedCircle = circle;
                    });
                    _updateAnnotations(); // highlight selected dot
                  } catch (e) {
            ...
            myLocationEnabled: widget.showMyLocationButton,
        ...
        if (widget.showMyLocationButton || _manualUpdateMode)
          Positioned(
            right: 16,
            top: 16,
            child: SafeArea(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_manualUpdateMode && widget.showMyLocationButton)
                    const SizedBox(height: 8),
                  if (_manualUpdateMode)
                    FloatingActionButton.small(
                      onPressed: _fetchForCurrentView,
                      tooltip: '手動更新地圖',
        ...
        // zoom controls (＋/－) placed above the filter button on the left
        Positioned(
          left: 16,
          bottom: 96, // a bit above the filter control
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              FloatingActionButton.small(
                    ...
                    if (_selected!.address != null) ...[
                      ...
                      Row(
                        children: [
                          ...
                          Expanded(child: Text(_selected!.address!)),
                        ],
                      ),
                    ],
